...
def light_test(ctx: m.AppContext, device: str | None) -> None:
    ...
    ctx.snapshot_manager.replace_config("light_test", m.Config(ctx.orc.Light, m.OFF), end, "light_test")
    time.sleep(10)
    report(expect_on=False)
    ctx.api.dispatch(m.Config(ctx.orc.Light, m.ON), force=True)
    time.sleep(10)
...
def sound_test(ctx: m.AppContext, device: str | None) -> None:
    ...
    ctx.api.dispatch(m.Configs(m.Config(ctx.orc.Chromecast, url)), force=True)
