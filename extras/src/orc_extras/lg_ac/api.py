...
class Query(enum.IntEnum):
    """TLV 0x1f5 query type sent to the device."""

    CAPABILITIES = 1  # the capabilities schema
    VALUES = 2  # current field values
...
def build_query(query: Query) -> bytes:
    # 0x1f5 selects the query type; header [1,1,2,2,1].
    return _build_frame(2, 2, 1, m.TLVField.of(0x1F5, query).encode())
